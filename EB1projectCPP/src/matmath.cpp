		       ...
		       m_elem wy, m_elem wz )
{
  int           i;
  m_elem  temp;
  m_elem  delta[QUATERNION_SIZE], newmat[QUATERNION_SIZE];
  ...
  newmat[0] = quat[0]*delta[0] - quat[1]*delta[1] - quat[2]*delta[2]
    - quat[3]*delta[3];
  newmat[1] = quat[1]*delta[0] + quat[0]*delta[1] - quat[3]*delta[2]
    + quat[2]*delta[3];
  newmat[2] = quat[2]*delta[0] + quat[3]*delta[1] + quat[0]*delta[2]
    - quat[1]*delta[3];
  newmat[3] = quat[3]*delta[0] - quat[2]*delta[1] + quat[1]*delta[2]
    + quat[0]*delta[3];

  /*  Finally, normalize the quaternion and copy it back  */

  temp = newmat[0]*newmat[0] + newmat[1]*newmat[1] + newmat[2]*newmat[2] + newmat[3]*newmat[3];
