function group = parseGroupsTorstenEdge(group,traj,name)


leT = length(traj);
leG1 = length(group);
pixelSize = 0.107;%0.107; %0.11; % (microns)
samplingRate = 0.39; %0.39 all but EB1+MTs(0.62) - Noco 1- if not -> 0.5; % (seconds)
...
% saveas(h,(['X:\AlexData\Torsten\trichostatinA\ctrl\cell2\groups\grouping_figure',name,'.fig']),'fig');
% close 
% Initialize 'growth' structure
parser=struct(...
...
% save(['X:\AlexData\Torsten\020608_nocodazole_titration_EB1\no_Noc#10\groups\parser',name],'parser') 
% save((['X:\AlexData\Torsten\020608_nocodazole_titration_EB1\no_Noc#10\groups\parameters',name,'.mat']),'pixelSize',...
%     'samplingRate','percentTracksGrouped','nbTracksLT4','nbGroups','nbFeaturesTrackedLT4',...
%     'gapsInSegment', 'probShrink','percentTimeShrink');
