function group = parseGroups(dirName)

minLT = 4;
delta_T_sh = [];speedShrink=[];

if nargin == 0
    [fileName,dirName] = uigetfile('*.tif','Choose an image');
end
load([dirName(1:end-12),filesep,'point_files',filesep,'config001_4p00_track_bidir.mat']); %5
% load([dirName(1:end-8),'\point_files\config001_10p00_track_bidir.mat']); %torsten
% load([dirName(1:end-8),'\groupLT4-15-15\group.mat']);
load([dirName(1:end-12),filesep,'groups1',filesep,'group.mat']);
...
leT = length(traj);
leG1 = length(group);
pixelSize = 0.076;%0.107; %0.11; % (microns)
samplingRate = 0.5;%0.39; %0.39 all but EB1+MTs(0.62) - Noco 1- if not -> 0.5; % (seconds)
...
for i = 1:leG % for all groups
    ...
    for j = 1:(leGi-1) % for all member-tracks of a group
        ...
        if cos_ang > 0 % growth
            ...
            %----------------------------------------------------------
%             if speedGrowth_GAP(gg) > CO
%                 plot([traj(group(i).list(j)).points(end,1),traj(group(i).list(j+1)).points(1,1)],[traj(group(i).list(j)).points(end,2),traj(group(i).list(j+1)).points(1,2)],'r-')
% 
%             else
%                 plot([traj(group(i).list(j)).points(end,1),traj(group(i).list(j+1)).points(1,1)],[traj(group(i).list(j)).points(end,2),traj(group(i).list(j+1)).points(1,2)],'b-')
%             end
            %--------------------------------------------------------
        end
...
saveas(h,([dirName(1:end-12),filesep,'groups1',filesep,'grouping_figure','.fig']),'fig');
close
% Initialize 'growth' structure
parser=struct(...
...
parser.shirnk_speeds_gaps = min(speedShrink,0);
...
% save([dirName(1:end-8),filesep,'group45-60',filesep,'group'],'group')
save([dirName(1:end-12),filesep,'groups1',filesep,'parser'],'parser')
save(([dirName(1:end-12),filesep,'groups1',filesep,'parameters','.mat']),'pixelSize',...
